...
class GerenciadorMotoristas:
    def __init__(self):
        ...
        self.arquivo_html = "relatorio_cgs.html"
    ...
    def carregar_html(self):
        ...
        try:
            if os.path.exists(self.arquivo_html):
                with open(self.arquivo_html, 'r', encoding='utf-8') as file:
                    self.conteudo_html = file.read()
                return True
            else:
                self.conteudo_html = None
                return False
    ...
    def salvar_html(self, conteudo):
        """Salva conteúdo HTML no arquivo"""
        try:
            with open(self.arquivo_html, 'w', encoding='utf-8') as file:
                file.write(conteudo)
            self.conteudo_html = conteudo
            return True
        except Exception as e:
            st.error(f"Erro ao salvar arquivo HTML: {e}")
            return False

    def importar_html(self, arquivo):
        """Importa arquivo HTML"""
        try:
            # Lê o conteúdo do arquivo
            conteudo = arquivo.getvalue().decode("utf-8")
            return self.salvar_html(conteudo)
        except Exception as e:
...
# Sidebar para navegação - ATUALIZADA COM NOVA OPÇÃO
...
pagina = st.sidebar.selectbox(
    ...
    ["📊 Dashboard", "👥 Cadastrar Motorista", "📤 Importar Excel", "✏️ Editar Motorista", "🗑️ Excluir Motorista", "📋 Lista Completa", 
     ...
     "📈 Relatório CGS"]  # NOVA OPÇÃO ADICIONADA
)
...
# Carrega HTML se existir
if gerenciador.conteudo_html is None:
    gerenciador.carregar_html()
        ...
        if motorista_selecionado:
            ...
            with col2:
                ...
                confirmacao = st.text_input(
                    ...
                    help="Digite exatamente 'EXCLUIR' para confirmar a exclusão"
                )
                ...
                if st.button("🗑️ Excluir Permanentemente", type="primary"):
                    if confirmacao == "EXCLUIR":
                        ...
                        st.error("❌ Confirmação incorreta. Digite 'EXCLUIR' para confirmar.")
    else:
...
# Página: Lista Completa
    ...
    if gerenciador.dados is not None and not gerenciador.dados.empty:
        st.info(f"Total de {len(gerenciador.dados)} motoristas cadastrados")
        ...
        if not colunas_exibicao:
            colunas_exibicao = COLUNAS_PRINCIPAIS
        
        # Exibir dados filtrados
        if not dados_filtrados.empty:
            st.dataframe(
                dados_filtrados[colunas_exibicao],
            ...
            with col1:
                st.metric("Total Filtrado", len(dados_filtrados))
            
            with col2:
                com_veiculo = len(dados_filtrados[dados_filtrados['com-veiculo'] == 'Sim'])
                st.metric("Com Veículo", com_veiculo)
            
            with col3:
                doc_vencido = len(dados_filtrados[dados_filtrados['doc-vencido'] == 'Sim'])
                st.metric("Docs Vencidos", doc_vencido)
            
            with col4:
                ativos = len(dados_filtrados[dados_filtrados['status'] == 'ATIVO'])
...
# Página: Lista de Clientes
    ...
    if gerenciador.tem_dados_clientes():
        st.info(f"Total de {len(gerenciador.dados_clientes)} clientes cadastrados")
...
# NOVA PÁGINA: Relatório CGS
elif pagina == "📈 Relatório CGS":
    st.title("📈 Relatório CGS")
    
    # Seção para importar arquivo HTML
    st.subheader("📤 Importar Arquivo HTML")
    
    arquivo_html = st.file_uploader(
        "Selecione o arquivo HTML para importar",
        type=['html'],
        help="Arquivo HTML com relatório CGS"
    )
    ...
    # Seção para visualizar HTML existente
    st.subheader("👁️ Visualizar Relatório HTML")
    
    if gerenciador.conteudo_html:
        # Opção para visualizar como HTML renderizado
        if st.checkbox("Visualizar como HTML renderizado"):
            st.components.v1.html(gerenciador.conteudo_html, height=600, scrolling=True)
        else:
...
st.sidebar.markdown(
    f"""
    **Última atualização:**  
    {gerenciador.ultima_atualizacao.strftime('%d/%m/%Y %H:%M:%S') if gerenciador.ultima_atualizacao else 'N/A'}
    
    **Total de motoristas:**  
    {len(gerenciador.dados) if gerenciador.dados is not None else 0}
    
    **Desenvolvido por**  
    Sistema de Gestão de Motoristas v1.0
    """
)