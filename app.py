...
class GerenciadorMotoristas:
    ...
    # MÉTODOS CORRIGIDOS PARA ASSOCIAÇÃO DE USUÁRIOS
    def obter_usuarios_motoristas(self):
        ...
        try:
            ...
            if self.dados is not None and not self.dados.empty:
                # Verifica se a coluna 'usuario' existe
                if 'usuario' not in self.dados.columns:
                    st.warning("Coluna 'usuario' não encontrada nos dados dos motoristas")
                    return []
                
                # Garante que a coluna 'usuario' seja tratada como string
                if self.dados['usuario'].dtype != 'object':
                    self.dados['usuario'] = self.dados['usuario'].astype(str)
                
                # Remove valores NaN e converte para string
                usuarios_series = self.dados['usuario'].dropna()
                
                # Converte para string e remove espaços em branco
                usuarios_series = usuarios_series.astype(str).str.strip()
                
                # Remove valores vazios e 'nan'
                usuarios_filtrados = usuarios_series[
                    (usuarios_series != '') & 
                    (usuarios_series.str.lower() != 'nan') &
                    (usuarios_series != 'None')
                ]
                
                # Remove duplicatas e retorna lista ordenada
                usuarios_unicos = sorted(usuarios_filtrados.unique().tolist())
                return usuarios_unicos
            else:
                return []
                
        except Exception as e:
            st.error(f"Erro ao obter usuários dos motoristas: {str(e)}")
            return []
    ...
    def obter_nome_por_usuario(self, usuario):
        ...
        try:
            if self.dados is not None and not self.dados.empty and 'usuario' in self.dados.columns:
                ...
                # Remove valores NaN e converte para string
                self.dados['usuario'] = self.dados['usuario'].fillna('').astype(str).str.strip()
                
                # Busca o motorista
                motorista = self.dados[self.dados['usuario'] == usuario_str]
                
                if not motorista.empty and 'nome' in motorista.columns:
            ...
            st.error(f"Erro ao obter nome por usuário: {str(e)}")
            return ""
...
# PÁGINAS PARA CLIENTES - SEÇÃO CORRIGIDA
    ...
    if gerenciador.dados is not None and not gerenciador.dados.empty:
        # Busca os usuários dos motoristas para o dropdown com tratamento de erro
        try:
            usuarios_motoristas = gerenciador.obter_usuarios_motoristas()
        except Exception as e:
            st.error(f"Erro ao carregar usuários dos motoristas: {str(e)}")
            usuarios_motoristas = []
                ...
                with col1:
                    ...
                    if usuario_selecionado:
                        try:
                            ...
                            st.error(f"Erro ao buscar motorista: {str(e)}")
                ...
                if submitted:
                    if cliente and usuario_selecionado and empresa:
                        ...
                        try:
                            ...
                            st.error(f"Erro ao obter nome do motorista: {str(e)}")
                            nome_motorista = ""
...
# ... (resto do código permanece igual - as outras páginas)
